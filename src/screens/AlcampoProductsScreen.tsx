...
import { fetchAlcampoProductsByCategory, type AlcampoProduct } from '../api/catalog';
import { getMeta } from '../constants/categoryMeta';
import { useThemedStyles } from '../context/ThemeContext';
import { useTranslation } from '../context/LanguageContext';
import { alcampoToUI } from '../lib/productAdapters';
import StoreProductList from '../components/StoreProductList';
import { useHeaderTopPadding } from '../hooks/useHeaderTopPadding';

type RouteProps = RouteProp<CatalogStackParamList, 'AlcampoProducts'>;

export default function AlcampoProductsScreen() {
  ...
  const [products, setProducts] = useState<AlcampoProduct[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    setError(false);
    fetchAlcampoProductsByCategory(categoryId)
      .then(setProducts)
      .catch(() => setError(true))
      .finally(() => setLoading(false));
  }, [categoryId]);

  const uiProducts = useMemo(() => products.map(alcampoToUI), [products]);
