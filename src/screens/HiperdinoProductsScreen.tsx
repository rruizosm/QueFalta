...
import { fetchHiperdinoProductsByCategory, type HiperdinoProduct } from '../api/catalog';
import { getMeta } from '../constants/categoryMeta';
import { useThemedStyles } from '../context/ThemeContext';
import { useTranslation } from '../context/LanguageContext';
import { hiperdinoToUI } from '../lib/productAdapters';
import StoreProductList from '../components/StoreProductList';
import { useHeaderTopPadding } from '../hooks/useHeaderTopPadding';

type RouteProps = RouteProp<CatalogStackParamList, 'HiperdinoProducts'>;

export default function HiperdinoProductsScreen() {
  ...
  const [products, setProducts] = useState<HiperdinoProduct[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    setError(false);
    fetchHiperdinoProductsByCategory(categoryId)
      .then(setProducts)
      .catch(() => setError(true))
      .finally(() => setLoading(false));
  }, [categoryId]);

  const uiProducts = useMemo(() => products.map(hiperdinoToUI), [products]);
