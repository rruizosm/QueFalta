...
import type { AmetllerProduct } from '../api/catalog';
...
interface Props {
  /** Producto a mostrar (ya cargado de ametller_products, con ficha). null = oculto. */
  product: AmetllerProduct | null;
  onClose: () => void;
...
/** Detalle de un producto de Ametller Origen. Pinta los datos ya cargados (el
 *  catálogo va por el espejo en Supabase). Ficha bilingüe es/ca (ingredientes/
 *  nutrición/conservación/origen); los alérgenos no se muestran (SFCC solo da
 *  códigos numéricos sin leyenda). */
export default function AmetllerProductModal({ product, onClose, topInset = 16, badgeLabel }: Props) {
  ...
  const fav = isProductFavorite('ametller', product.id);
  ...
  return (
    <View style={styles.overlay}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <ProductDetailImage uri={product.thumbnail} style={styles.photo} badgeLabel={badgeLabel} alertTarget={{ store: 'ametller', productId: product.id }} />

        <Text style={styles.name}>{product.displayName}</Text>

        <ProductPriceLine store="ametller" productId={product.id} price={price} />
        {product.pricePerUnit ? <Text style={styles.refPrice}>{product.pricePerUnit}</Text> : null}

        <ProductDetailDiscoverySection
          nutrition={nutrition}
          productId={product.id}
          excludeStore="ametller"
        />
        ...
        <Text style={styles.note}>{t('product.fromStore', { store: 'Ametller Origen' })}</Text>
      </ScrollView>
