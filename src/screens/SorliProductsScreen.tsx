...
import { fetchSorliProductsByCategory, type SorliProduct } from '../api/catalog';
import { getMeta } from '../constants/categoryMeta';
import { useThemedStyles } from '../context/ThemeContext';
import { useTranslation } from '../context/LanguageContext';
import { sorliToUI } from '../lib/productAdapters';
import StoreProductList from '../components/StoreProductList';
import ActiveCartBanner from '../components/ActiveCartBanner';

type RouteProps = RouteProp<CatalogStackParamList, 'SorliProducts'>;

export default function SorliProductsScreen() {
  const styles = useThemedStyles(themedStyles);
  const { t } = useTranslation();
  const navigation = useNavigation<any>();
  const { categoryId, categoryName, parentName } = useRoute<RouteProps>().params;

  const [products, setProducts] = useState<SorliProduct[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    setError(false);
    fetchSorliProductsByCategory(categoryId)
      .then(setProducts)
      .catch(() => setError(true))
      .finally(() => setLoading(false));
  }, [categoryId]);

  const uiProducts = useMemo(() => products.map(sorliToUI), [products]);
