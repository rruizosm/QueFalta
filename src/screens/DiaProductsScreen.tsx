...
import { fetchDiaProductsByCategory, type DiaProduct } from '../api/catalog';
...
type DiaProductsRouteProp = RouteProp<CatalogStackParamList, 'DiaProducts'>;
...
export default function DiaProductsScreen() {
  ...
  const { categoryId, categoryName, parentName } = useRoute<DiaProductsRouteProp>().params;
  ...
  const [detail, setDetail] = useState<DiaProduct | null>(null);
  ...
  const renderGridItem = ({ item }: { item: DiaProduct }) => (
    <ProductGridCard
      width={CARD_W}
      uri={item.thumbnail}
  ...
  const renderItem = ({ item }: { item: DiaProduct }) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <DiaProductModal product={detail} onClose={() => setDetail(null)} />
    </View>
  );
}
