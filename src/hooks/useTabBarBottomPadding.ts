...
import { FloatingPagerContext } from '../components/bottom-tabs-pager/PagerGestureContext';
 ...
 *
 * - Dentro del pager real: la barra flota en todas las plataformas y se suma
 *   su altura efectiva, incluido el espacio hasta el safe area inferior.
 * - Fuera del pager, sin cristal (Android, iOS ≤ 18, build sin el módulo de expo-glass-effect):
...
export function useTabBarBottomPadding(designPadding: number): number {
  ...
  const floatingPager = useContext(FloatingPagerContext);
  return floatingPager || glassAvailable ? designPadding + tabBarHeight : designPadding;
}
