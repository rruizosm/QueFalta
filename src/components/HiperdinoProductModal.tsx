...
import type { HiperdinoProduct } from '../api/catalog';
...
/** Detalle de un producto de HiperDino. Pinta los datos ya cargados (el catálogo
 *  va por el espejo en Supabase). HiperDino no da ficha (ingredientes/nutrición)
 *  ni marca/formato aparte → solo la categoría, como Aldi. */
export default function HiperdinoProductModal({ product, onClose, topInset = 16, badgeLabel }: Props) {
  ...
  const fav = isProductFavorite('hiperdino', product.id);
  ...
  return (
    <View style={styles.overlay}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <ProductDetailImage uri={product.thumbnail} style={styles.photo} badgeLabel={badgeLabel} alertTarget={{ store: 'hiperdino', productId: product.id }} />
        ...
        <SimilarProductsSection productId={product.id} excludeStore="hiperdino" />

        {/* Características del producto. HiperDino no expone ficha (ingredientes/
        ...
        <Text style={styles.note}>{t('product.fromStore', { store: 'HiperDino' })}</Text>
      </ScrollView>
