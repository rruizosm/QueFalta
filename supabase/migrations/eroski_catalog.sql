-- Espejo del catálogo de Eroski en Supabase (CATÁLOGO + BÚSQUEDA).
-- Lo rellena scripts/sync-eroski.mjs 1×/semana. Tabla aparte (modelo "una tabla
...
create table if not exists public.eroski_categories (
...
create table if not exists public.eroski_products (
...
alter table public.eroski_products alter column first_seen_at set default now();

create index if not exists eroski_products_category_idx      on public.eroski_products (category_id);
create index if not exists eroski_products_category_ids_idx  on public.eroski_products using gin (category_ids);
create index if not exists eroski_products_name_trgm_idx     on public.eroski_products using gin (display_name gin_trgm_ops);
create index if not exists eroski_products_norm_trgm_idx     on public.eroski_products using gin (display_name_norm gin_trgm_ops);
create index if not exists eroski_products_first_seen_idx    on public.eroski_products (first_seen_at desc);
create index if not exists eroski_products_price_changed_idx on public.eroski_products (price_changed_at desc) where price_changed_at is not null;

drop trigger if exists track_price_change on public.eroski_products;
create trigger track_price_change
  before update of unit_price on public.eroski_products
  for each row execute function public.catalog_track_price_change();

alter table public.eroski_products   enable row level security;
alter table public.eroski_categories enable row level security;

drop policy if exists "eroski catalog read" on public.eroski_products;
create policy "eroski catalog read"
on public.eroski_products for select to anon, authenticated using (true);

drop policy if exists "eroski categories read" on public.eroski_categories;
create policy "eroski categories read"
on public.eroski_categories for select to anon, authenticated using (true);
