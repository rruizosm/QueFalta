...
import type { CondisProduct } from '../api/catalog';
...
interface Props {
  /** Producto a mostrar (ya cargado de condis_products). null = oculto. */
  product: CondisProduct | null;
  onClose: () => void;
...
export default function CondisProductModal({ product, onClose, topInset = 16, badgeLabel }: Props) {
  ...
  const fav = isProductFavorite('condis', product.id);
  ...
  return (
    <View style={styles.overlay}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <ProductDetailImage uri={product.thumbnail} style={styles.photo} badgeLabel={badgeLabel} alertTarget={{ store: 'condis', productId: product.id }} />
        ...
        <Text style={styles.note}>{t('product.fromStore', { store: 'Condis' })}</Text>
      </ScrollView>
