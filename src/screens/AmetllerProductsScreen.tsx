...
import { fetchAmetllerProductsByCategory, type AmetllerProduct } from '../api/catalog';
import { getMeta } from '../constants/categoryMeta';
import { useThemedStyles } from '../context/ThemeContext';
import { useTranslation } from '../context/LanguageContext';
import { ametllerToUI } from '../lib/productAdapters';
import StoreProductList from '../components/StoreProductList';
import { useHeaderTopPadding } from '../hooks/useHeaderTopPadding';

type RouteProps = RouteProp<CatalogStackParamList, 'AmetllerProducts'>;

export default function AmetllerProductsScreen() {
  ...
  const [products, setProducts] = useState<AmetllerProduct[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    setError(false);
    fetchAmetllerProductsByCategory(categoryId)
      .then(setProducts)
      .catch(() => setError(true))
      .finally(() => setLoading(false));
  }, [categoryId]);

  const uiProducts = useMemo(() => products.map(ametllerToUI), [products]);
