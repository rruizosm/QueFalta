...
- ⏳ **Nota de salud estilo Yuka (Plus, solo Mercadona)** (2026-06-13): backend hecho — `scripts/lib/health-score.mjs` (Nutri-Score 2023 + aditivos + 0-100, testeado), `scripts/extract-mercadona-nutrition.mjs` (lee la foto de la etiqueta trasera `photos[1].zoom` con visión Claude Haiku → tabla por 100g → score), migración `mercadona_health.sql`, workflow `mercadona-nutrition.yml`. Pendiente: ejecutar migración, añadir `ANTHROPIC_API_KEY` (.env.local + secret GitHub), lanzar backfill, y **Fase 3 UI** (sección de salud en ProductDetailModal gateada a Plus). Ver `scripts/README-mercadona-nutrition.md`.
