# Lanza el sync de bonÀrea desde tu PC (pensado para una Tarea Programada diaria).
# bonÀrea NO está tras Cloudflare, así que esto TAMBIÉN puede correr en GitHub Actions;
# este runner es para ejecutarlo en local junto al de Carrefour. Ver README-bonarea-sync.md.
#
...
$log = Join-Path $logDir ("bonarea-sync-{0}.log" -f (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
# --- Ejecutar el sync ---
Set-Location $repo
"=== bonArea sync $(Get-Date -Format 'u') ===" | Tee-Object -FilePath $log
& node scripts/sync-bonarea.mjs *>&1 | Tee-Object -FilePath $log -Append
$code = $LASTEXITCODE
if ($code -eq 0 -and $env:DRY_RUN -ne '1') {
  "=== Actualizando comparador (bonarea) ===" | Tee-Object -FilePath $log -Append
  $env:STORES = 'bonarea'
...
Get-ChildItem $logDir -Filter 'bonarea-sync-*.log' |
