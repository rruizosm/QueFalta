# Ejecuta el sync de Eroski desde tu PC. Eroski/Caprabo bloquean las IPs de
...
$log = Join-Path $logDir ("eroski-sync-{0}.log" -f (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Set-Location $repo
"=== Eroski sync $(Get-Date -Format 'u') ===" | Tee-Object -FilePath $log
