...
import { fetchPlusfrescProductsByCategory, type PlusfrescProduct } from '../api/catalog';
...
export default function PlusfrescProductsScreen() {
  ...
  const [products, setProducts] = useState<PlusfrescProduct[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    setError(false);
    fetchPlusfrescProductsByCategory(categoryId, profile?.postalCode ?? null)
      .then(setProducts)
      .catch(() => setError(true))
      .finally(() => setLoading(false));
  }, [categoryId, profile?.postalCode]);

  const uiProducts = useMemo(() => products.map(plusfrescToUI), [products]);
