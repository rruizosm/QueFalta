# Lanza el sync de Sorli desde tu PC (alternativa local al workflow sync-sorli.yml).
# Sorli necesita un navegador para firmar la sesión: en local usa el Chrome del
# sistema (PW_CHANNEL=chrome, sin instalar Chromium de Playwright). Ver
# README-sorli-sync.md.
#
...
$log = Join-Path $logDir ("sorli-sync-{0}.log" -f (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
$env:CONCURRENCY           = '4'
$env:PW_CHANNEL            = 'chrome'   # usa el Chrome del sistema para firmar la sesión

# --- Ejecutar el sync ---
Set-Location $repo
"=== Sorli sync $(Get-Date -Format 'u') ===" | Tee-Object -FilePath $log
& node scripts/sync-sorli.mjs *>&1 | Tee-Object -FilePath $log -Append
$code = $LASTEXITCODE
if ($code -eq 0 -and $env:DRY_RUN -ne '1') {
  "=== Actualizando comparador (sorli) ===" | Tee-Object -FilePath $log -Append
  $env:STORES = 'sorli'
...
Get-ChildItem $logDir -Filter 'sorli-sync-*.log' |
