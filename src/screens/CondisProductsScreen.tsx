...
import { fetchCondisProductsByCategory, type CondisProduct } from '../api/catalog';
...
export default function CondisProductsScreen() {
  ...
  const [products, setProducts] = useState<CondisProduct[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    setError(false);
    fetchCondisProductsByCategory(categoryId)
      .then(setProducts)
      .catch(() => setError(true))
      .finally(() => setLoading(false));
  }, [categoryId]);

  const uiProducts = useMemo(() => products.map(condisToUI), [products]);
