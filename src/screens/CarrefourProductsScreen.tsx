...
type CarrefourProductsRouteProp = RouteProp<CatalogStackParamList, 'CarrefourProducts'>;
...
export default function CarrefourProductsScreen() {
  ...
  const { categoryId, categoryName, parentName } = useRoute<CarrefourProductsRouteProp>().params;
  ...
  const [detail, setDetail] = useState<CarrefourProduct | null>(null);
  ...
  const renderGridItem = ({ item }: { item: CarrefourProduct }) => (
    <ProductGridCard
      width={CARD_W}
      uri={item.thumbnail}
  ...
  const renderItem = ({ item }: { item: CarrefourProduct }) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <CarrefourProductModal product={detail} onClose={() => setDetail(null)} />
    </View>
  );
}
