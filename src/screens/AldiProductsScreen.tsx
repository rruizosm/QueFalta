...
import { fetchAldiProductsByCategory, type AldiProduct } from '../api/catalog';
import { getMeta } from '../constants/categoryMeta';
import { useThemedStyles } from '../context/ThemeContext';
import { useTranslation } from '../context/LanguageContext';
import { aldiToUI } from '../lib/productAdapters';
import StoreProductList from '../components/StoreProductList';
import ActiveCartBanner from '../components/ActiveCartBanner';

type RouteProps = RouteProp<CatalogStackParamList, 'AldiProducts'>;

export default function AldiProductsScreen() {
  const styles = useThemedStyles(themedStyles);
  const { t } = useTranslation();
  const navigation = useNavigation<any>();
  const { categoryId, categoryName, parentName } = useRoute<RouteProps>().params;

  const [products, setProducts] = useState<AldiProduct[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    setError(false);
    fetchAldiProductsByCategory(categoryId)
      .then(setProducts)
      .catch(() => setError(true))
      .finally(() => setLoading(false));
  }, [categoryId]);

  const uiProducts = useMemo(() => products.map(aldiToUI), [products]);
