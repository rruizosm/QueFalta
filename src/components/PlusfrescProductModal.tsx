...
import type { PlusfrescProduct } from '../api/catalog';
...
interface Props {
  /** Producto a mostrar (ya cargado de plusfresc_products, con ficha). null = oculto. */
  product: PlusfrescProduct | null;
  onClose: () => void;
...
export default function PlusfrescProductModal({ product, onClose, topInset = 16, badgeLabel }: Props) {
  ...
  // Plusfresc no publica EAN: el índice se calcula con la tabla nutricional que
  // llega del espejo (`nutrition` o `nutrition_ca`, ya seleccionada por el
  // adaptador según el idioma activo).
  ...
  const fav = isProductFavorite('plusfresc', product.id);
  ...
  return (
    <View style={styles.overlay}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <ProductDetailImage uri={product.thumbnail} style={styles.photo} badgeLabel={badgeLabel} alertTarget={{ store: 'plusfresc', productId: product.id }} />

        <Text style={styles.name}>{product.displayName}</Text>

        <ProductPriceLine store="plusfresc" productId={product.id} price={price} />
        {product.pricePerUnit ? <Text style={styles.refPrice}>{product.pricePerUnit}</Text> : null}

        <ProductDetailDiscoverySection
          nutrition={nutrition}
          productId={product.id}
          excludeStore="plusfresc"
        />
        ...
        <Text style={styles.note}>{t('product.fromStore', { store: 'Plusfresc' })}</Text>
      </ScrollView>
