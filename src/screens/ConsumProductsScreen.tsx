...
import { fetchConsumProductsByCategory, type ConsumProduct } from '../api/catalog';
...
type ConsumProductsRouteProp = RouteProp<CatalogStackParamList, 'ConsumProducts'>;
...
export default function ConsumProductsScreen() {
  const styles = useThemedStyles(themedStyles);
  const navigation = useNavigation<any>();
  const { categoryId, categoryName, parentName } = useRoute<ConsumProductsRouteProp>().params;
  ...
  const [detail, setDetail] = useState<ConsumProduct | null>(null);
  ...
  const renderGridItem = ({ item }: { item: ConsumProduct }) => (
    <ProductGridCard
      width={CARD_W}
      uri={item.thumbnail}
  ...
  const renderItem = ({ item }: { item: ConsumProduct }) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <ConsumProductModal product={detail} onClose={() => setDetail(null)} />
    </View>
  );
}
